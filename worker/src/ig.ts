...
export async function fetchProfile(page: Page, username: string): Promise<IgProfile> {
  const result = await page.evaluate(async (u) => {
    ...
    if (!user?.id) return { __error: "sem_user" };
    return {
  ...
  if ((result as { __error?: string }).__error) {
    throw new Error("perfil: " + (result as { __error: string }).__error);
  }
...
export async function fetchFollowersPage(
  ...
  const result = await page.evaluate(
    ...
    { pkParam: pk, maxIdParam: maxId },
  );

  if ((result as { __error?: string }).__error) {
    const code = (result as { __error: string }).__error;
    const err = new Error(code);
    (err as Error & { code?: string }).code = code.startsWith("RATE")
      ? "RATE"
